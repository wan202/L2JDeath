...
public class Q334_TheWishingPotion extends Quest
{
	...
	public Q334_TheWishingPotion()
	{
		...
		setItemsIds(ALCHEMY_TEXT_ID, SECRET_BOOK_ID, AMBER_SCALE_ID, WIND_SOULSTONE_ID, GLASS_EYE_ID, HORROR_ECTOPLASM_ID, SILENOS_HORN_ID, ANT_SOLDIER_APHID_ID, TYRANTS_CHITIN_ID, BUGBEAR_BLOOD_ID);
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("30738-16.htm"))
		{
			if (st.getQuestItemsCount(WISH_POTION_ID) >= 1)
			{
				...
				if (Rnd.chance(50))
				{
					addSpawn(SUCCUBUS_OF_SEDUCTION, player, false, 0, true);
					addSpawn(SUCCUBUS_OF_SEDUCTION, player, false, 0, true);
					addSpawn(SUCCUBUS_OF_SEDUCTION, player, false, 0, true);
				}
				else
				{
					addSpawn(RUPINA, player, false, 0, true);
				}
			}
			else
			{
		...
		else if (event.equalsIgnoreCase("30738-17.htm"))
		{
			if (st.getQuestItemsCount(WISH_POTION_ID) >= 1)
			{
				...
				if (WISH_CHANCE <= 33)
				{
					addSpawn(GRIMA, player, false, 0, true);
					addSpawn(GRIMA, player, false, 0, true);
					addSpawn(GRIMA, player, false, 0, true);
				...
				{
					st.giveItems(57, (int) ((Rnd.get(10) + 1) * 1000000L));
				}
				else
				{
					addSpawn(GRIMA, player, false, 0, true);
		...
		else if (event.equalsIgnoreCase("30738-18.htm"))
		{
			if (st.getQuestItemsCount(WISH_POTION_ID) >= 1)
			{
				...
				{
					addSpawn(SANCHES, player, false, 0, true);
				}
			}
			else
			{
		...
		else if (event.equalsIgnoreCase("30738-19.htm"))
		{
			if (st.getQuestItemsCount(WISH_POTION_ID) >= 1)
			{
				...
				{
					addSpawn(WISDOM_CHEST, player, false, 0, true);
				}
			}
			else
			{
	...
	public String onKill(Npc npc, Player player, boolean isSunnon)
	{
		...
		{
			addSpawn(BONAPARTERIUS, player, false, 0, true);
		}
		else if ((npcId == BONAPARTERIUS) && Rnd.chance(51))
		{
			addSpawn(RAMSEBALIUS, player, false, 0, true);
		}
		else if ((npcId == RAMSEBALIUS) && Rnd.chance(51))
		{
			addSpawn(GREAT_DEMON_KING, player, false, 0, true);
		}
